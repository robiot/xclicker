...
void click_handler(gpointer *data) {
  ...
  while (isClicking) {
    ...
    case CLICK_TYPE_SINGLE:
      printf("%d", args->multiclick_interval);
      if (args->multiclick) {
    ...
    case CLICK_TYPE_DOUBLE:
      if (args->multiclick) {
        for (int i = 0; i < (args->multiclick_times); i++) {
          ...
          usleep(args->multiclick_interval);
        }
      } else {
        if (click(display, args->button, using_xevent, hold_type_ms) == FALSE)
          xapp_error("Sending click", -1);
...
void multiclick_check_toggle(GtkToggleButton *check) {
  ...
  gtk_widget_set_sensitive(mainappwindow.multiclick_times_entry,
                           gtk_toggle_button_get_active(check));

  gtk_widget_set_sensitive(mainappwindow.multiclick_interval_entry,
                           gtk_toggle_button_get_active(check));
  g_key_file_set_boolean(config_gfile, PCK_MULTICLICK,
                         gtk_toggle_button_get_active(check));
...
void input_changed_save_handler(GtkEditable *editable,
  ...
  if (editable == GTK_EDITABLE(mainappwindow->hours_entry)) {
    ...
    g_key_file_set_string(config_gfile, PCK_MULTICLICK_TIMES, text);
             ...
             GTK_EDITABLE(mainappwindow->multiclick_interval_entry)) {
...
void start_clicked() {
  ...
  if (strcmp(click_type_text, "Single") == 0){
      ...
      gtk_widget_set_sensitive(mainappwindow.multiclick_check, FALSE);
  ...
  if ((data->multiclick = gtk_toggle_button_get_active(
           GTK_TOGGLE_BUTTON(mainappwindow.multiclick_check)))) {
    data->multiclick_times =
        get_text_to_int(mainappwindow.multiclick_times_entry);
    data->multiclick_interval =
        get_text_to_int(mainappwindow.multiclick_interval_entry);
  }
...
void mainappwindow_import_config() {
  ...
  gtk_entry_set_text_if_not_null(mainappwindow.repeat_entry,
                                 config->repeat_times);
  gtk_toggle_button_set_active(mainappwindow.multiclick_check,
                               config->use_multiclick);
  gtk_entry_set_text_if_not_null(mainappwindow.multiclick_times_entry,
                                 config->multiclick_times);
  gtk_entry_set_text_if_not_null(mainappwindow.multiclick_interval_entry,
                                 config->multiclick_interval);
...
static void main_app_window_class_init(MainAppWindowClass *class) {
  ...
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(class), MainAppWindow,
                                       hours_entry);
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(class), MainAppWindow,
                                       multiclick_times_entry);
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(class), MainAppWindow,
                                       multiclick_interval_entry);
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(class), MainAppWindow,
                                       minutes_entry);
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(class), MainAppWindow,
                                       seconds_entry);
